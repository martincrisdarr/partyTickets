...
export const removeSeller = createAsyncThunk('sellers/removeSeller', async (payload) => {
  try {
    const response = await axios.delete(`/sellers/${payload}`);
    return response.data;
  } catch (e) {
    const axiosError = e.response?.data;
    return Promise.reject(axiosError.message);
...
export const sellersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //DELETE SELLER
    builder.addCase(removeSeller.fulfilled, (state) => {
      state.message = 'Success';
      state.isFetching = false;
      toast.success('Eliminada con éxito');
    });
    builder.addCase(removeSeller.pending, (state) => {
      state.isFetching = true;
    });
